...
load(paste0(data_subpath,"/TCGA_input.Rdata"))
...
#Select patient
metatime <- filter(meta)

#ConsensusClusterPlus with selected genes
exprSet_genes=exprSet[(row.names(exprSet) %in% genes) == TRUE, str_sub(colnames(exprSet),1,12) %in% row.names(metatime)]
exprSet_genes = sweep(exprSet_genes, 1, apply(exprSet_genes,1,median,na.rm=T))


TCGA_outs_path <- paste0(outs_subpath,"/S7A_consensus_matrix/TCGA")
dir.create(TCGA_outs_path, recursive=TRUE)
title=TCGA_outs_path
...
survival_outs_dir <- paste0(outs_subpath,"/7B_S7B_survival/TCGA")
...
for (i in 2:2){	
  ...
  ggsurvplot(sfit1,
             ...
             font.legend = 12,
             ggtheme = theme_base())
  ggsave(paste0(survival_outs_dir,"/TCGA_signature_KMplot_ConsensusClusterPlus", i, ".pdf"), width = 4, height = 4)
  print(pairwise_survdiff(Surv(time, event)~group, data=y, p.adjust.method = "BH", rho = 0))
  
  #gene_patient heatmap
  x=exprSet[(row.names(exprSet) %in% genes) == TRUE, str_sub(colnames(exprSet),1,12) %in% rownames(y)]
  y=y[match(str_sub(colnames(x),1,12),y$ID),]
  all(y$ID==str_sub(colnames(x),1,12))
  ...
  pdf(paste0(survival_outs_dir,"/TCGA_signature_KMplot_ConsensusClusterPlus", i, "_hm.pdf"), width = 12, height = 11)
  print(Heatmap(test, 
  ...
  pdf(paste0(survival_outs_dir,"/TCGA_signature_KMplot_ConsensusClusterPlus", i, "_box.pdf"), width = 3.5, height = 5)
